...
if __name__ == "__main__":
    ...
    card_copies = []
    wins = []
    for k, line in enumerate(inp.splitlines()):
        card_copies = sorted(card_copies)
        line_no = k + 1
        i = 0
        ...
        for number in win_numbers:
            if number in my_numbers:
                i += 1
        if i > 0:
            card_copies.extend(range(line_no + 1, line_no + i + 1))
            wins.append(list(range(line_no + 1, line_no + i + 1)))
        else:
            wins.append([])
        total += 1
    for copy in card_copies:
        card_copies.extend(wins[copy - 1])
        total += 1
    print(total)
